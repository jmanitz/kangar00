...
author: "Juliane Manitz and Stefanie Friedrichs"
date: "`r Sys.Date()`"
...
Once a `GWASdata` and an appropriate `pathway` objects are created, we can start to calculate kernel matrices to test a pathways influence on disease risk. 
...
# Pathway information
pathw_shell <- pathway(id='hsa04022', adj=matrix(0), sign=as.vector(matrix(0)[matrix(0)!=0]))
hsa04022 <- get_network_matrix(pathw_shell, directed=FALSE)
```

The package `kangar00` offers three different kernel functions to compute a similarity matrix for the individuals in analysis. 
...
``` {r}
K_lin <- lin_kernel(my_gwas, hsa04022, calculation='cpu') 
summary(K_lin)
```
...
``` {r}
K_sia <- sia_kernel(my_gwas, hsa04022, calculation='cpu')
summary(K_sia)
```
will return a quadratic `matrix` of dimension equal to the number of individuals
in the `GWASdata` object.
...
``` {r}
K_net <- net_kernel(my_gwas, hsa04022, calculation='cpu')
summary(K_net)
```
will return a quadratic `matrix` of dimension equal to the number of individuals
in the `GWASdata` object.
...
``` {r}
K <- calc_kernel(my_gwas, hsa04022, type='lin', parallel='none')
```
...
``` {r}
lkmt_test(pheno ~ 1+sex+age, K, my_gwas, method='satt')
lkmt_test(pheno ~ 1+sex+age, K, my_gwas, method='davies')
```
