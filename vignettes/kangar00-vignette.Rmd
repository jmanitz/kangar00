...
?get_network_matrix # add example
``` 
...
Genome-wide association study data from a case control study is basis for a test of a pathway's influence on a disease risk with the logistic kernel machine test. In `kangar00`, all needed information is captured by the S4 `GWASdata` object, which can be constructed by 
...
K_lin <- calc_kernel(gwas, hsa04022, type='lin')
plot(K_lin)
```
