...
results. Via mapping of SNPs to genes forming a network, associations of 
specific pathways with disease risk can be analysed.  
...
incorporation of covariates. Furthermore, they are computationally efficient.
The model for a logistic kernel machine regression of a pathway on a binary
outcome is given by
...
*  If you do not want to update your SNP information, you have to create a `snp_info`
   object including a data frame listing all SNPs to be annotated. This data frame 
...
*   A value for argument `parallel` to decide how the kernel should be 
    calculated. Options are `none` for sequential, `cpu` for parallel and `gpu`
    for gpu calculation are available. Currently only parallel computation is 
    implemented. 
...
*   A value for argument `parallel` to decide how the kernel should be 
    calculated. Currently only `none` for sequential calculation is available.  
