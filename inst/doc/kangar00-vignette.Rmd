...
*   Genotype data for each individual. 
       ...
       *  Genotype data needs to be one line per individual and in 
          `big.matrix` format, which is a memory efficient matrix object as 
          specified in the `big.memory` package (currently only availyble for 
          unix / linux systems). 
...
ADD READ_GENO FUNCTION HERE WEHEN FINISHED         
...
```
K_lin <- lin_kernel(gwas, p, parallel='none')
```
...
```
K_sia <- sia_kernel(gwas, p, parallel='gpu')
```
will return a quadratic `matrix` of dimension equal to the number of individuals
in the `GWASdata` object.
...
```
K_net <- net_kernel(gwas, p, parallel='none')
```
will return a quadratic `matrix` of dimension equal to the number of individuals
in the `GWASdata` object.
